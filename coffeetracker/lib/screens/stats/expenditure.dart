...
class ExpenditurePage extends StatelessWidget {
  ...
  ExpenditurePage({super.key, required this.startDate, required this.endDate, required this.range});
  ...
  Future<Map<String, dynamic>> _fetchConsumptionData() async {
    ...
    double totalPrice = 0;
    Map<String, double> coffeeChoices = {
      'Purchased Coffee': 0,
      'Homemade Coffee': 0,
      'Vending Machine': 0,
    };

    Map<String, Map<String, double>> coffeeTypes = {
      'Purchased Coffee': {},
      'Homemade Coffee': {},
      'Vending Machine': {},
    };
    ...
    for (var record in records) {
      ...
      if (recordDate.isAfter(startDate) && recordDate.isBefore(endDate)) {
        ...
        coffeeChoices[coffeeChoice] = (coffeeChoices[coffeeChoice] ?? 0) + price;
        coffeeTypes[coffeeChoice]?[coffeeType] = (coffeeTypes[coffeeChoice]?[coffeeType] ?? 0) + price;
      }
    }
      ...
      body: FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            final coffeeChoices = data['coffeeChoices'] as Map<String, double>;
            final coffeeTypes = data['coffeeTypes'] as Map<String, Map<String, double>>;
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  _buildPieChart(coffeeChoices, totalPrice),
  ...
  Widget _buildPieChart(Map<String, double> coffeeChoices, double totalPrice) {
    ...
    if (totalPrice == 0) {
      ...
      sections = coffeeChoices.entries.map((entry) {
        final value = (entry.value / totalPrice) * 100;
              ...
              children: coffeeChoices.entries.map((entry) {
                final color = entry.key == 'Purchased Coffee'
                    ...
                    : entry.key == 'Homemade Coffee'
                        ? const Color.fromARGB(255, 159, 222, 108)
                        : const Color.fromARGB(255, 118, 215, 239);
                final percentage = totalPrice == 0 ? 0.0 : (entry.value / totalPrice) * 100;
                return Row(
                  children: [
                    Container(
                      width: 15,
  ...
  Widget _buildCoffeeChoiceList(Map<String, double> coffeeChoices, Map<String, Map<String, double>> coffeeTypes) {
    return Padding(
        ...
        children: coffeeChoices.keys.map((choice) {
          final color = choice == 'Purchased Coffee'
              ...
              : choice == 'Homemade Coffee'
                  ? const Color.fromARGB(255, 159, 222, 108)
                  : const Color.fromARGB(255, 118, 215, 239);
          // Sort coffeeTypes by price.
          List<MapEntry<String, double>> sortedEntries = coffeeTypes[choice]!.entries.toList()..sort((a, b) => b.value.compareTo(a.value));
          return Column(
            children: [
              Row(
                children: [
                  ...
                  Text.rich(
                    TextSpan(
                      children: [
                        TextSpan(
                          text: coffeeChoices[choice]!.toStringAsFixed(2),
                          style: GoogleFonts.montserrat(
                            fontSize: 16,
                            fontWeight: FontWeight.bold,
                          ),
                    ...
                    return Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Text(
                            '${entry.value.toStringAsFixed(2)} \tGBP',
                            style: GoogleFonts.montserrat(
                              fontSize: 14,
                              fontWeight: FontWeight.normal,
                            ),
