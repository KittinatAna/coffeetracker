...
class MonthlyInsight extends StatelessWidget {
  ...
  MonthlyInsight({super.key, required this.startDate, required this.endDate, required this.range});
  ...
  Future<Map<String, dynamic>> _fetchConsumptionData() async {
    ...
    int totalVolume = 0;
    int thisMonthTotalVolume = 0;

    Map<DateTime, Map<String, int>> monthlyConsumption = {};

    for (var record in records) {
      DateTime recordDate = DateTime.parse(record['date']);
      if (recordDate.isAfter(startDate) && recordDate.isBefore(endDate)) {
        int thisMonthVolume = record['volume'] ?? 0;
        thisMonthTotalVolume += thisMonthVolume;
      }

      DateTime startOfMonth = DateTime(recordDate.year, recordDate.month, 1);
      DateTime last3Months = DateTime(endDate.year, endDate.month - 3, 0);

      if (startOfMonth.isBefore(endDate) && startOfMonth.isAfter(last3Months)) {
        ...
        if (!monthlyConsumption.containsKey(startOfMonth)) {
          monthlyConsumption[startOfMonth] = {
            'Purchased Coffee': 0,
            'Homemade Coffee': 0,
            'Vending Machine': 0,
            'Total': 0,
        ...
        monthlyConsumption[startOfMonth]![coffeeChoice] = (monthlyConsumption[startOfMonth]![coffeeChoice] ?? 0) + volume;
        monthlyConsumption[startOfMonth]!['Total'] = (monthlyConsumption[startOfMonth]!['Total'] ?? 0) + volume;
      }
    }

    return {
      'thisMonthTotalVolume': thisMonthTotalVolume,
      ...
      body: FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            final thisMonthTotalVolume = data['thisMonthTotalVolume'] as int;
            final totalVolume = data['totalVolume'] as int;
            final monthlyConsumption = data['monthlyConsumption'] as Map<DateTime, Map<String, int>>;
            final averageVolume = (data['totalVolume'] as int) / 3;

            List<_ChartData> trendData = [];
            List<DateTime> last3Months = List.generate(3, (i) => DateTime(startDate.year, startDate.month - 1 + i, 1));
            for (int i = 0; i < last3Months.length; i++) {
              DateTime month = last3Months[i];
              Map<String, int> consumption = monthlyConsumption[DateTime(month.year, month.month, 1)] ?? {'Purchased Coffee': 0, 'Homemade Coffee': 0, 'Vending Machine': 0, 'Total': 0};
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  _buildStackedColumnChart(monthlyConsumption),
                  ...
                  const SizedBox(height: 20),
                  _buildRecommendation(thisMonthTotalVolume, totalVolume),
                ],
              ),
            );
          }
  ...
  Widget _buildStackedColumnChart(Map<DateTime, Map<String, int>> monthlyConsumption) {
  ...
  Widget _buildRecommendation(int thisMonthTotalVolume, int totalVolume) {
    return Column(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ...
                'Over the course of a month, this equates to a maximum of 28,800 mL.',
                style: GoogleFonts.montserrat(
                  fontSize: 14,
                  fontWeight: FontWeight.normal,
                ),
