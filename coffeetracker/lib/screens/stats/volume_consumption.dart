...
class VolumeConsumptionPage extends StatelessWidget {
  final DateTime startDate;
  final DateTime endDate;
  final String range;

  VolumeConsumptionPage({super.key, required this.startDate, required this.endDate, required this.range});
  ...
  Future<Map<String, dynamic>> _fetchConsumptionData() async {
    ...
    for (var record in records) {
      ...
      if (recordDate.isAfter(startDate) && recordDate.isBefore(endDate)) {
        ...
        coffeeChoices[coffeeChoice] = (coffeeChoices[coffeeChoice] ?? 0) + volume;
        coffeeTypes[coffeeChoice]?[coffeeType] = (coffeeTypes[coffeeChoice]?[coffeeType] ?? 0) + volume;
      }
    }
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  _buildPieChart(coffeeChoices, totalVolume),
  ...
  Widget _buildPieChart(Map<String, int> coffeeChoices, int totalVolume) {
    ...
    if (totalVolume == 0) {
      ...
      sections = coffeeChoices.entries.map((entry) {
        final value = (entry.value / totalVolume) * 100;
              ...
              children: coffeeChoices.entries.map((entry) {
                final color = entry.key == 'Purchased Coffee'
                    ...
                    : entry.key == 'Homemade Coffee'
                        ? const Color.fromARGB(255, 159, 222, 108)
                        : const Color.fromARGB(255, 118, 215, 239);
                final percentage = totalVolume == 0 ? 0.0 : (entry.value / totalVolume) * 100;
                return Row(
                  children: [
                    Container(
                      width: 15,
