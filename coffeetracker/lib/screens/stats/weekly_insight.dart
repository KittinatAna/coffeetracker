...
class WeeklyInsight extends StatelessWidget {
  ...
  WeeklyInsight({super.key, required this.startDate, required this.endDate, required this.range});
  ...
  Future<Map<String, dynamic>> _fetchConsumptionData() async {
    ...
    int totalVolume = 0;
    int thisWeekTotalVolume = 0;

    // Remove minute of startdate and enddate
    DateTime endDatewitheout = DateTime(endDate.year, endDate.month, endDate.day);

    Map<DateTime, Map<String, int>> weeklyConsumption = {};

    for (var record in records) {
      DateTime recordDate = DateTime.parse(record['date']);
      DateTime startOfWeek = recordDate.subtract(Duration(days: recordDate.weekday - 1));

      if (recordDate.isAfter(startDate) && recordDate.isBefore(endDatewitheout)) {
        int thisWeekVolume = record['volume'] ?? 0;
        thisWeekTotalVolume += thisWeekVolume;
      }

      if (startOfWeek.isAfter(startDate.subtract(const Duration(days: 27))) && startOfWeek.isBefore(endDatewitheout)) {
        ...
        if (!weeklyConsumption.containsKey(startOfWeek)) {
          weeklyConsumption[startOfWeek] = {
            'Purchased Coffee': 0,
            'Homemade Coffee': 0,
            'Vending Machine': 0,
            'Total': 0,
        ...
        weeklyConsumption[startOfWeek]![coffeeChoice] = (weeklyConsumption[startOfWeek]![coffeeChoice] ?? 0) + volume;
        weeklyConsumption[startOfWeek]!['Total'] = (weeklyConsumption[startOfWeek]!['Total'] ?? 0) + volume;
      }
    }

    return {
      'thisWeekTotalVolume': thisWeekTotalVolume,
  ...
  String _getFormattedDateRange() {
    if (range == 'week') {
      ...
      DateTime endOfWeek = endDate.add(const Duration(days: -1));
      ...
      body: FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            final thisWeekTotalVolume = data['thisWeekTotalVolume'] as int;
            final totalVolume = data['totalVolume'] as int;
            final weeklyConsumption = data['weeklyConsumption'] as Map<DateTime, Map<String, int>>;
            final averageVolume = (data['totalVolume'] as int) / 4;

            // Remove minute of startdate and enddate
            DateTime endDatewitheout = DateTime(endDate.year, endDate.month, endDate.day);

            List<_ChartData> trendData = [];
            List<DateTime> last4Weeks = List.generate(4, (i) => endDatewitheout.add(const Duration(days: -28)).add(Duration(days: 7 * i)));
            for (int i = 0; i < last4Weeks.length; i++) {
              DateTime date = last4Weeks[i];
              Map<String, int> consumption = weeklyConsumption[date] ?? {'Purchased Coffee': 0, 'Homemade Coffee': 0, 'Vending Machine': 0, 'Total': 0};
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  _buildStackedColumnChart(weeklyConsumption),
                  ...
                  const SizedBox(height: 10),
                  _buildRecommendation(thisWeekTotalVolume, totalVolume),
                ],
              ),
            );
          }
  ...
  Widget _buildStackedColumnChart(Map<DateTime, Map<String, int>> weeklyConsumption) {
  ...
  Widget _buildRecommendation(int thisWeekTotalVolume, int totalVolume) {
    return Column(
          ...
          Column(
            ...
            children: [
              ...
              Text(
                ...
                'Over the course of a week, this equates to a maximum of 6,720 mL.',
                style: GoogleFonts.montserrat(
                  fontSize: 14,
                  fontWeight: FontWeight.normal,
                ),
