...
class YearlyInsight extends StatelessWidget {
  ...
  YearlyInsight({super.key, required this.startDate, required this.endDate, required this.range});
  ...
  Future<Map<String, dynamic>> _fetchConsumptionData() async {
    ...
    int totalVolume = 0;
    int thisYearTotalVolume = 0;

    Map<int, Map<String, int>> yearlyConsumption = {};

    for (var record in records) {
      DateTime recordDate = DateTime.parse(record['date']);
      if (recordDate.isAfter(startDate) && recordDate.isBefore(endDate)) {
        int thisYearVolume = record['volume'] ?? 0;
        thisYearTotalVolume += thisYearVolume;
      }

      int recordYear = recordDate.year;
      if (recordYear >= startDate.year && recordYear <= endDate.year) {
        ...
        if (!yearlyConsumption.containsKey(recordYear)) {
          yearlyConsumption[recordYear] = {
            'Purchased Coffee': 0,
            'Homemade Coffee': 0,
            'Vending Machine': 0,
            'Total': 0,
        ...
        yearlyConsumption[recordYear]![coffeeChoice] = (yearlyConsumption[recordYear]![coffeeChoice] ?? 0) + volume;
        yearlyConsumption[recordYear]!['Total'] = (yearlyConsumption[recordYear]!['Total'] ?? 0) + volume;
      }
    }

    return {
      'thisYearTotalVolume': thisYearTotalVolume,
  ...
  String _getFormattedDateRange() {
    if (range == 'year') {
      DateTime startOfYear = DateTime(startDate.year, startDate.month + 2, 0);
      ...
      body: FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            final thisYearTotalVolume = data['thisYearTotalVolume'] as int;
            final totalVolume = data['totalVolume'] as int;
            final yearlyConsumption = data['yearlyConsumption'] as Map<int, Map<String, int>>;
            final averageVolume = (data['totalVolume'] as int) / 3;

            List<_ChartData> trendData = [];
            List<int> last3Years = List.generate(3, (i) => startDate.year - 1 + i);
            for (int i = 0; i < last3Years.length; i++) {
              int year = last3Years[i];
              Map<String, int> consumption = yearlyConsumption[year] ?? {'Purchased Coffee': 0, 'Homemade Coffee': 0, 'Vending Machine': 0, 'Total': 0};
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  _buildStackedColumnChart(yearlyConsumption),
                  ...
                  const SizedBox(height: 20),
                  _buildRecommendation(thisYearTotalVolume, totalVolume),
                ],
              ),
            );
          }
  ...
  Widget _buildStackedColumnChart(Map<int, Map<String, int>> yearlyConsumption) {
  ...
  Widget _buildRecommendation(int thisYearTotalVolume, int totalVolume) {
    return Column(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ...
                'Over the course of a year, this equates to a maximum of 350,400 mL.',
                style: GoogleFonts.montserrat(
                  fontSize: 14,
                  fontWeight: FontWeight.normal,
                ),
